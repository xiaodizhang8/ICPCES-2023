...
def get_precondition_assert(problem, candidate_plan, action_map):
    ...
    for i in range(1, plan_length+1):
        ...
        step_args = step[1:]
        action = action_map['(' + step_name + ' ' + ' '.join(step_args) + ')']
        res += '(assert (= cpces_valcond'+str(i)+' '+Conjunction(action.all_precondition).to_smt(plan_length-i)+'))\n'
        declared_predicate = Conjunction(action.all_precondition).get_predicate(plan_length-i, declared_predicate)
