...

{
  ...
  /* header
   */
  printf("(define (problem cube-%d-%d)", gn, gg);
  printf("\n(:domain cube)");
  ...
  printf("\n(:init");
  for ( i = 0; i < gn - 1; i++ ) printf("\n(x-above p%d p%d)", i + 2, i + 1);
  for ( i = 0; i < gn - 1; i++ ) printf("\n(y-above p%d p%d)", i + 2, i + 1);
  for ( i = 0; i < gn - 1; i++ ) printf("\n(z-above p%d p%d)", i + 2, i + 1);
  for ( i = 0; i < gn; i++ ) printf("\n(cpt (x-pos p%d) %f)", 
				    i + 1, ((float) gprobability[0][i])/1000000.0);
  printf("\n(multi");
  for ( i = 0; i < gn; i++ ) printf("\n(x-pos p%d)", i + 1);
  printf("\n)");
  for ( i = 0; i < gn; i++ ) printf("\n(cpt (y-pos p%d) %f)", 
				    i + 1, ((float) gprobability[1][i])/1000000.0);
  printf("\n(multi");
  for ( i = 0; i < gn; i++ ) printf("\n(y-pos p%d)", i + 1);
  printf("\n)");
  for ( i = 0; i < gn; i++ ) printf("\n(cpt (z-pos p%d) %f)", 
				    i + 1, ((float) gprobability[2][i])/1000000.0);
  printf("\n(multi");
  for ( i = 0; i < gn; i++ ) printf("\n(z-pos p%d)", i + 1);
  printf("\n)");
...

{
  ...
  printf("-n <num>    number of positions (minimal 2)\n");
